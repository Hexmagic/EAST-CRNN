...
class custom_dataset(data.Dataset):
	def __init__(self, img_path, gt_path, scale=0.25, length=512):
		...
		self.length = length
		self.vl_map = {}
		for ele in self.gt_files:
			with open(ele, 'r') as f:
				lines = f.readlines()
				veritices, labels = extract_vertices(lines)
				self.vl_map[ele]=[veritices,labels]
	...
	def __getitem__(self, index):
		vertices, labels = self.vl_map[self.gt_files[index]]
...
if __name__ == '__main__':
	trainset = custom_dataset('train_img', 'train_gt')
	loader = data.DataLoader(trainset, batch_size=32,
                                        shuffle=True, num_workers=16, drop_last=True)
	from tqdm import tqdm
	for ele in tqdm(loader):
		pass
